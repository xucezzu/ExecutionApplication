 ...
 * 版权所有 © 徐长策
 */
public class ExecCircleProgressBar extends View {
    /**
     * 进度条最大值，默认为100
     */
     ...
     * 渐变圆周颜色数组
     */
    private int[] colorArray = new int[]{Color.parseColor("#000000"), Color.parseColor("#000000")};
     ...
     * @param context
     */
    public ExecCircleProgressBar(Context context) {
        this(context, null);
    }
     ...
     * @param attrs   自定义属性
     */
    public ExecCircleProgressBar(Context context, @Nullable AttributeSet attrs) {
     ...
     */
    public ExecCircleProgressBar(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (attr) {
                ...
                case R.styleable.circleProgressBar_circleWidth:
                    // 默认圆弧宽度为6dp
                    circleWidth = ta.getDimensionPixelSize(attr, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 6, getResources().getDisplayMetrics()));
                    break;
                default:
                    break;
            }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawText(canvas, center, radius, currentText);
    }
     ...
     * @param radius 圆的半径
     */
    private void drawText(Canvas canvas, int center, int radius, String text) {
        // 计算进度
     ...
     * 设置进度条中间要显示的字段
     *
     * @param currentText 进度条中间显示的文字
     */
    public void setCenterText(String currentText) {
